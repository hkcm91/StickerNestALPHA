...
export class CanvasPage {
   ...
   */
  async dragEntity(entityId: string, dx: number, dy: number): Promise<void> {
    const entity = this.page.locator(`[data-entity-id="${entityId}"]`);
   ...
   */
  async getEntityPosition(entityId: string): Promise<{ x: number; y: number }> {
    const entity = this.page.locator(`[data-entity-id="${entityId}"]`);
   ...
   */
  async getViewportState(): Promise<{ panX: number; panY: number; zoom: number }> {
    return await this.page.evaluate(() => {
      // This will be replaced with actual store access when canvas is implemented
      const viewport = (window as unknown as { __SN_VIEWPORT__?: { panX: number; panY: number; zoom: number } }).__SN_VIEWPORT__;
  ...
  async getAllEntityIds(): Promise<string[]> {
    ...
    return await entities.evaluateAll((els) =>
      els.map((el) => el.getAttribute('data-entity-id') || '').filter(Boolean)
    );
  }
  ...
  async isEditMode(): Promise<boolean> {
    return await this.page.evaluate(() => {
      // This will be replaced with actual store access when canvas is implemented
      const mode = (window as unknown as { __SN_CANVAS_MODE__?: 'edit' | 'preview' }).__SN_CANVAS_MODE__;
      return mode === 'edit';
    });
  }
 ...
 */
export const test = base.extend<{ canvasPage: CanvasPage }>({
